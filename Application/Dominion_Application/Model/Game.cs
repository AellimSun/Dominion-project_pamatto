    ...
    class Game
    {
        ...
        public void clickHand(string now, int idx)
        {
            ...
            else if (deck.HandDeck[idx].kind.Equals("money"))
            {
                ...
                if (deck.HandDeck[idx].Name.Equals("silver") && merchantUsed)
                {
                    gameTable.Coin++;
                    merchantUsed = false;
                }
            ...
            else if (deck.HandDeck[idx].kind.Equals("action"))
            {
                if (now.Equals("Action End"))
                {
                    if (gameTable.ActionNumber == 0) return;
        ...
        {
            if (gameTable.Coin >= cardList[i].price && gameTable.BuyNumber >= 1 && cardList[i].amount > 0)
            {
        ...
        {
            if (gameTable.Coin >= cardList[i].price && cardList[i].amount > 0)
            {
                gameTable.Coin = 0;
                market.SellCard(cardList[i]);
                deck.BuyCard(cardList[i]);
        ...
        public Card buyCSCard(int i)
        {
            ...
            if (gameTable.Coin >= list[i].price && gameTable.BuyNumber >= 1 && list[i].amount > 0)
            {
        ...
        public Card notBuyCSCard(int i)
        {
            List<Card> list = null;
            if (i < 3)
            {
            ...
            if (gameTable.Coin >= list[i].price && list[i].amount > 0)
            {
                gameTable.Coin = 0;
                market.SellCard(list[i]);
                deck.BuyCard(list[i]);
